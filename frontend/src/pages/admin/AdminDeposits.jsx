    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[var(--nb-muted)] mt-2">Approve or reject user-submitted deposits. See each user's chosen payment method at a glance.</p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className="bg-[var(--nb-card)] border-[var(--nb-border)] rounded-xl p-4">
          <div className="text-[10px] uppercase tracking-widest text-[var(--nb-muted)]">Approved value</div>
        ...
        <Card className="bg-[var(--nb-card)] border-[var(--nb-border)] rounded-xl p-4">
          <div className="text-[10px] uppercase tracking-widest text-[var(--nb-muted)]">Rows</div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {TABS.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm border capitalize ${
              tab === t ? "bg-[#0055FF] text-white border-[#0055FF]" : "border-[var(--nb-border)] text-[var(--nb-muted)] hover:text-white"
            }`}
          >
            {t}
          </button>
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 rounded-md border border-[var(--nb-border)] px-2 h-9 text-xs text-[var(--nb-muted)]">
            ...
            >
              {METHODS.map((m) => <option key={m.k} value={m.k} className="bg-[var(--nb-card)]">{m.label}</option>)}
            </select>
          </div>
          <div className="relative">
            <Search className="w-3.5 h-3.5 absolute left-2.5 top-3 text-[var(--nb-muted)]" />
            <Input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Name, phone, ref…"
              data-testid="deposit-search-input"
              className="pl-8 h-9 bg-[var(--nb-card2)] border-[var(--nb-border)] text-white w-56"
            />
          </div>
        </div>
      </div>
      ...
      <Card className="bg-[var(--nb-card)] border-[var(--nb-border)] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--nb-border)]">
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="text-center py-8 text-[var(--nb-muted)]" data-testid="no-deposits-admin">No deposits match your filters.</td></tr>
            )}
            {filtered.slice(0, visible).map((d) => (
              <React.Fragment key={d.id}>
                <tr data-testid={`admin-deposit-${d.id}`} className="align-top">
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="w-6 h-6 rounded border border-[var(--nb-border)] grid place-items-center text-[var(--nb-muted)] hover:text-white"
                      aria-label="Expand row"
                    >
                      {expanded[d.id] ? <ChevronUp className="w-3 h-3"/> : <ChevronDown className="w-3 h-3"/>}
                    </button>
                  </td>
                  <td className="px-4 py-3 text-[var(--nb-muted)] whitespace-nowrap">{new Date(d.created_at).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <div>{d.user_name}</div>
                    <div className="text-xs text-[var(--nb-muted)] tabular">{d.user_phone}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-[var(--nb-muted)] max-w-[160px] truncate" title={d.reference || d.merchant_order_no || ""}>
                ...
                {expanded[d.id] && (
                  <tr className="bg-[var(--nb-page)]/70" data-testid={`admin-deposit-detail-${d.id}`}>
                    <td colSpan={8} className="px-6 py-4">
                      <div className="grid md:grid-cols-2 gap-4 text-xs">
                        <div className="space-y-1.5">
                          <div className="text-[10px] uppercase tracking-widest text-[var(--nb-muted)]">Deposit</div>
...
function Kv({ k, v, copyable }) {
  return (
    <div className="flex items-start gap-2">
      <span className="text-[var(--nb-muted)] w-32 shrink-0">{k}</span>
      <span className="text-white tabular flex-1 break-all">{v || "—"}</span>
      {copyable && v && (
        <button onClick={() => copyText(v)} className="text-[var(--nb-muted)] hover:text-white shrink-0" aria-label="Copy">
