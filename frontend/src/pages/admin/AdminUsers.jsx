...
import { Search, Eye, Users2 } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[#94A3B8] mt-2">Search users and open each one's live dashboard to view, credit or debit their wallet.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="rounded-lg border border-[#1A2B44] bg-[#0B1524] px-3 py-2 text-xs">
      ...
      <Card className="bg-[#0B1524] border-[#1A2B44] rounded-xl overflow-hidden">
        ...
        <LoadMore shown={Math.min(visible, users.length)} total={users.length} onMore={setVisible} step={20} testid="load-more-users" />
      </Card>
    </div>
  );
}
